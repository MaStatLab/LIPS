lips_hd_pred = function( Y, X, T = NULL, kstep = 0, alpha = NULL, method = "g", nparticle = 100, rho0=NULL, rho0.method ="beta-binomial",rho.prop=0.1,p.mix=6,resample.param=0,n.top.vars=5){
  ...
  ans = .Call('modelsel_pf_hd_pred_C', Y.centered, X.centered, p, as.integer(T), as.integer(kstep), as.numeric(alpha), method.int, as.integer(nparticle), as.numeric(rho0), rho0.method.int, as.numeric(rho.prop), as.numeric(p.mix),as.numeric(resample.param),as.integer(min(n.top.vars,T)))
