lips_hd = function( y.vec, x.mat, T = 10, kstep = 0, alpha = NA, method = "g", nparticle = 100, rho0=0.5, rho0.method ="const",rho.prop=0.1,p.mix=1,resample.param=1,n.top.vars=5){
  ...
  ans = .Call('modelsel_pf_hd_C', y.vec.centered, x.mat.centered, p, as.integer(T), as.integer(kstep), as.numeric(alpha), method.int, as.integer(nparticle), as.numeric(rho0), rho0.method.int, as.numeric(rho.prop), as.numeric(p.mix),as.numeric(resample.param),as.integer(min(n.top.vars,T)))

  names(ans)=c("logweights","ave_incl_prob","size","logBF")
